...
import { criarIndicacaoExterna } from "@/lib/supabase-helpers";
...
const Externo = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { consultor } = await criarIndicacaoExterna(formData);
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="text-center mb-8 animate-fade-in">
          ...
          <p className="text-muted-foreground">
            Formulário para síndicos e zeladores indicarem clientes
          </p>
        </div>
        ...
        <Card className="shadow-lg animate-fade-in" style={{ animationDelay: "0.1s" }}>
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit} className="space-y-8">
              {/* Dados do Síndico/Zelador */}
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b">
                  <div className="p-2 rounded-lg bg-primary/10">
                    <Building2 className="w-4 h-4 text-primary" />
                  </div>
                  <h2 className="font-semibold text-lg text-foreground">Dados do Síndico / Zelador</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="nome_sindico" className="flex items-center gap-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({ ...formData, nome_sindico: e.target.value })}
                      onBlur={() => handleBlur("nome_sindico")}
                      className={`transition-all focus:ring-2 focus:ring-primary/20 ${getFieldValidationClass("nome_sindico", formData.nome_sindico)}`}
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="condominio" className="flex items-center gap-1">
                    ...
                    <Input
                      ...
                      onBlur={() => handleBlur("condominio")}
                      className={`transition-all focus:ring-2 focus:ring-primary/20 ${getFieldValidationClass("condominio", formData.condominio)}`}
                    />
                  </div>
                </div>
              </div>
